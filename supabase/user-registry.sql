...
alter table public.user_registry add column if not exists phone text not null default '';
alter table public.user_registry add column if not exists phone_confirmed_at timestamptz;
...
after insert or update of email, email_confirmed_at, phone, phone_confirmed_at, last_sign_in_at on auth.users
