...
(async()=>{
  ...
  try{
    ...
    assert.ok(await dialog.locator('[data-alarm-file]').isDisabled(),'file also requires explicit consent');
    ...
    await page.evaluate(()=>{window.dispatchEvent(new Event('estudiemos:account-change'));window.dispatchEvent(new Event('estudiemos:account-ready'));});
    assert.match(await dialog.locator('[data-alarm-native-status]').textContent(),/todavía está pendiente/,'same-account sync must preserve the next activation step');
    ...
    await dialog.locator('.alarm-setup-file summary').click();
    const pendingDownload=page.waitForEvent('download');
    await dialog.locator('[data-alarm-file]').click();
    const download=await pendingDownload;
    assert.equal(download.suggestedFilename(),'Conectar-Alarmas-Estudiemos.estudiemos-alarmas');
    assert.deepEqual(JSON.parse(fs.readFileSync(await download.path(),'utf8')),{version:1,token:'pilot.signature'});
    assert.equal(await page.evaluate(()=>nativeLinks.length),2,'file connection does not use an external browser protocol');
    assert.match(await dialog.locator('[data-alarm-native-status]').textContent(),/todavía no está confirmada/);
    await page.setViewportSize({width:390,height:844});
    await dialog.locator('[data-alarm-file-download]').scrollIntoViewIfNeeded();
    assert.ok(await dialog.evaluate(el=>el.scrollWidth<=el.clientWidth+1),'mobile setup has no horizontal overflow');
    await page.screenshot({path:'tmp/ui-check/alarm-connection-file-mobile.png'});
    await page.setViewportSize({width:1280,height:720});
    await page.evaluate(()=>{window.EstudiemosAccount.getUser=()=>({id:'other-pilot'});window.dispatchEvent(new Event('estudiemos:account-change'));});
    assert.ok(await retry.isHidden(),'account changes must discard the old activation link');
    assert.equal(await retry.getAttribute('href'),null);
    assert.equal(await dialog.locator('[data-alarm-file-download]').getAttribute('href'),null);
    assert.ok(await dialog.locator('[data-alarm-activate]').isDisabled(),'new account requires its own consent');
    ...
    assert.equal(calls.length,3);assert.equal(calls[0].consent,true);assert.equal(calls[1].consent,true);assert.equal(calls[2].proof,'receipt');
