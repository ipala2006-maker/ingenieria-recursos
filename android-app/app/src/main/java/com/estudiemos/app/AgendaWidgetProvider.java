...
public class AgendaWidgetProvider extends AppWidgetProvider {
    ...
    static final String ACTION_COMPLETE = "com.estudiemos.app.COMPLETE_AGENDA_ITEM";
    static final String EXTRA_ITEM_ID = "agenda_item_id";
    ...
    private static void updateWidget(Context context, AppWidgetManager manager, int appWidgetId) {
        ...
        views.setRemoteAdapter(R.id.agenda_widget_list, serviceIntent);
        views.setEmptyView(R.id.agenda_widget_list, R.id.widget_empty);

        Intent completeIntent = new Intent(context, AgendaWidgetProvider.class).setAction(ACTION_COMPLETE);
        PendingIntent completeTemplate = PendingIntent.getBroadcast(
                context,
                5000 + appWidgetId,
                completeIntent,
                PendingIntent.FLAG_UPDATE_CURRENT | PendingIntent.FLAG_MUTABLE
        );
        views.setPendingIntentTemplate(R.id.agenda_widget_list, completeTemplate);

        PendingIntent openAgenda = openAgendaIntent(context);
        views.setOnClickPendingIntent(R.id.widget_header, openAgenda);
        views.setOnClickPendingIntent(R.id.widget_footer, openAgenda);

        int total = readUpcomingEntries(context, today).size();
        views.setTextViewText(
                R.id.widget_footer,
                total == 0 ? "Tocá para abrir Inbox" : total + (total == 1 ? " pendiente · Abrir Inbox" : " pendientes · Abrir Inbox")
        );
    ...
    private static void notifyDataChanged(Context context) {
        ...
        manager.notifyAppWidgetViewDataChanged(ids, R.id.agenda_widget_list);
    }
    ...
    static final class AgendaEntry {
        final String id;
        final String title;
